...
{
	begin();			// FTLK: rozpoczęcie dołączania nowych Fl_Wiget do tego okna
		w.attach(*this);	//  Widget tworzy swoje Fl_Wiget
	end();				// FTLK: koniec dołączania nowych Fl_Wiget do tego okna
}
...
{
		for (unsigned int i = shapes.size(); 0<i; --i)	// ostatni dołączony zostanie zwolniony pierwszy
			if (shapes[i-1]==&s)
